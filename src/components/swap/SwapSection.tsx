...
import { Settings, ArrowDown, ChevronDown, Route, Wallet, BarChart3, Search, X, PlusCircle, Lock, Unlock, History, Zap, Clock, Fuel, ShieldAlert } from 'lucide-react';
...
export function SwapSection() {
  ...
  const priceImpact = quote?.estimate?.fromAmountUSD && quote?.estimate?.toAmountUSD ? 
    ((parseFloat(quote.estimate.fromAmountUSD) - parseFloat(quote.estimate.toAmountUSD)) / parseFloat(quote.estimate.fromAmountUSD)) * 100 : 0;
  ...
  return (
    <div className="w-full max-w-[480px] mx-auto space-y-4">
      ...
      <GlassCard className="p-4 flex justify-between items-center bg-[#0a1224] border-white/5 shadow-lg rounded-3xl">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm font-black text-white">${dbStats.totalVolume.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</p>
          </div>
        </div>
      ...
      <GlassCard className="p-2 overflow-hidden border-[#00F0FF]/20 shadow-2xl rounded-3xl bg-[#0a1224]">
          ...
          {showSettings && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="px-4 pb-2">
              <div className="flex gap-2">
                {['0.1', '0.5', '1.0'].map(val => (
                  <button key={val} onClick={() => setSlippage(val)} className={`flex-1 py-1.5 rounded-lg text-xs font-bold transition-all ${slippage === val ? 'bg-[#00F0FF] text-black' : 'bg-zinc-900 text-zinc-400 border border-zinc-700'}`}>{val}%</button>
                ))}
                <input type="number" value={slippage} onChange={(e) => setSlippage(e.target.value)} className="w-20 bg-zinc-900 border border-zinc-700 rounded-lg px-2 text-xs text-white outline-none" placeholder="Custom" />
              </div>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div className="bg-[#050b14] p-4 rounded-[22px] m-1 border border-zinc-800/50 hover:border-zinc-700 transition-colors">
          ...
          <div className="flex justify-between items-center gap-4">
            <input type="number" placeholder="0.0" disabled={isAmountLocked} value={amount} onChange={(e) => setAmount(e.target.value)} className="bg-transparent text-4xl font-black text-white outline-none w-full disabled:opacity-50" />
            <button onClick={() => setSelectingToken('from')} className="flex items-center gap-2 bg-[#00F0FF]/10 hover:bg-[#00F0FF]/20 px-3 py-2 rounded-2xl transition-all shrink-0 shadow-sm border border-[#00F0FF]/20">
          ...
          <div className="flex justify-between items-center mt-4">
            ...
            <div className="flex items-center gap-3">
              <span className="text-xs text-zinc-500 font-mono">Bal: {fromBalance ? Number(fromBalance.formatted).toFixed(4) : '0.00'}</span>
        ...
        <div className="relative h-1 flex justify-center items-center z-10 my-1">
          <button onClick={() => { setFromToken(toToken); setToToken(fromToken); setQuote(null); }} className="bg-[#0a1224] p-2.5 rounded-xl border-4 border-[#0a1224] text-[#00F0FF] bg-zinc-800 hover:text-white hover:rotate-180 transition-all duration-300">
        ...
        <div className="bg-[#050b14] p-4 rounded-[22px] m-1 border border-zinc-800/50 hover:border-zinc-700 transition-colors">
          ...
          <div className="flex justify-between items-center gap-4">
            <input type="text" disabled value={quote ? formatUnits(BigInt(quote.estimate.toAmount), toToken.decimals) : ''} placeholder={isFetchingQuote ? "Routing..." : "0.0"} className="bg-transparent text-4xl font-black text-white/50 outline-none w-full" />
            <button onClick={() => setSelectingToken('to')} className="flex items-center gap-2 bg-zinc-800 hover:bg-zinc-700 px-3 py-2 rounded-2xl transition-all shrink-0 shadow-sm border border-zinc-700">
          ...
          <div className="flex justify-between items-center mt-4">
            ...
            {quote && <div className="text-sm font-bold text-zinc-400">≈ ${parseFloat(quote.estimate.toAmountUSD).toFixed(2)}</div>}
          </div>
        </div>

        <AnimatePresence>
          {quote && !isFetchingQuote && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="mt-2 mx-1 p-3 bg-zinc-900/50 rounded-2xl border border-zinc-800 text-sm">
        ...
        <div className="p-1 mt-2">
          <Button onClick={handleSwap} disabled={isSwapping || isFetchingQuote || !amount} className="w-full py-5 text-lg font-black bg-gradient-to-r from-[#00F0FF] to-[#B026FF] text-white shadow-lg hover:opacity-90 rounded-2xl tracking-widest">
      ...
      <div className="px-2 pt-2">
        <div className="flex items-center gap-2 text-zinc-500 mb-3"><History className="w-4 h-4 text-[#00F0FF]"/> <span className="text-xs font-bold uppercase tracking-wider">Recent Activity</span></div>
        {dbStats.history.length === 0 ? <p className="text-xs text-zinc-600 italic text-center p-4 border border-zinc-800 rounded-xl bg-[#050b14]">No swaps recorded in database.</p> : (
          <div className="space-y-2 max-h-40 overflow-y-auto custom-scrollbar pr-1">
            {dbStats.history.map((tx:any, i:number) => (
              <div key={i} className="flex justify-between items-center bg-[#0a1224] p-3 rounded-xl border border-zinc-800/50 hover:bg-zinc-800 transition-colors">
                ...
                <a href={`https://basescan.org/tx/${tx?.tx_hash}`} target="_blank" rel="noreferrer" className="text-[10px] text-[#00F0FF] hover:underline font-mono">${Number(tx?.usd_value || 0).toFixed(2)}</a>
              </div>
            ))}
          </div>
        )}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
            <GlassCard className="w-full max-w-sm p-4 flex flex-col h-[600px] max-h-[80vh] bg-[#0a1224] border-[#00F0FF]/20 shadow-2xl rounded-3xl">
              <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={() => { setSelectingToken(null); setSearchQuery(''); setCustomTokenResult(null); }} className="text-zinc-500 hover:text-white"><X className="w-5 h-5" /></button>
              </div>
